...
type Set = Array<{ count: number, color: string }>;
type Game = { game: number, sets: Array<Set> };
...
const concatHands = (hands: Set) => hands.reduce((acc, { count, color }) => ({...acc, [color]: Math.max(acc[color] ?? 0, count)}), {} as Record<string, number>)
